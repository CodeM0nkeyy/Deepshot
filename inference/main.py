...
from inference.model import ContextAwareModel
from inference.train import test
...
def main(args):
    ...
    if args.GPU >= 0 and torch.cuda.is_available():
        ...
        # Check for Apple MPS (Metal Performance Shaders) device
    ...
    test_loader = torch.utils.data.DataLoader(dataset_Test,
        ...
        num_workers=0, pin_memory=True,pin_memory=False,prefetch_factor=None)

    
    # Load the best model and compute its performance
    checkpoint = torch.load(os.path.join("models", "model.pth.tar"), map_location=device)
    # checkpoint = torch.load(os.path.join("models", "model.pth.tar"))
